...
export default function PageEditor() {
  ...
  if (!page) return <div className="text-ink-600">Loading page content...</div>;
    ...
    <form onSubmit={save} className="pb-8">
      <div className="sticky top-0 z-20 -mx-6 lg:-mx-10 px-6 lg:px-10 py-4 mb-6 bg-slate-100/95 backdrop-blur border-b border-slate-200">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <Link to="/admin/pages" className="text-sm text-ink-500 hover:text-ink-800">
              ← All pages
            </Link>
            <h1 className="font-serif text-3xl font-bold text-ink-800 mt-1">Edit {page.title}</h1>
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-4 py-2.5 rounded-lg border border-slate-300 bg-white text-ink-800 font-medium hover:bg-slate-50"
            >
              View page
            </a>
            <button
      ...
      {message && (
        <div className="mb-6 text-sm text-ink-800 bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-3">
          {message}
        </div>
      )}
        ...
        <nav className="lg:sticky lg:top-28 space-y-1">
          ...
          {outline.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
              className="block text-sm text-ink-600 hover:text-ink-900 px-3 py-2 rounded-lg hover:bg-white"
            >
              {item.label}
            </a>
          ))}
        ...
        <div className="space-y-8">
          <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <label className="block">
              <span className="block text-sm font-semibold text-ink-800 mb-1.5">CMS page name</span>
              <input
                value={page.title}
          ...
          {Editor ? (
            ...
            <p className="text-ink-600">No editor is available for this page yet.</p>
          )}
        </div>
      </div>
    </form>
