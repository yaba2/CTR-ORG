      ...
      <div className="site-container">
        <div className="py-12 lg:py-16">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10">
            ...
            {Object.entries(footerLinks).map(([title, links]) => (
              <div key={title}>
                <h3 className="font-serif font-semibold text-sm mb-4 text-white">{title}</h3>
                <ul className="space-y-2.5">
                  {links.map((link) => (
                    <li key={link.name}>
                      <Link
                        to={link.path}
                        className="text-white text-sm hover:text-gold-200 transition-colors flex items-center gap-1 group"
                      >
        ...
        <div className="border-t border-navy-800 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6">
            <Link to="/about" className="text-white text-sm hover:text-gold-200 transition-colors">
              Privacy Policy
            </Link>
            <Link to="/about" className="text-white text-sm hover:text-gold-200 transition-colors">
              Terms of Service
            ...
            <Link
              to="/admin/login"
              className="px-3.5 py-1.5 rounded-lg border border-white text-white text-sm font-medium hover:bg-white hover:text-ink-900 transition-colors"
            >
              Admin Login
            </Link>
          </div>
        ...
        <div className="border-t border-navy-800 py-3 text-center">
          <p className="text-white text-xs sm:text-sm">
            ...
            <a href="tel:0904515006" className="hover:text-gold-200 transition-colors">
              0904515006
            </a>
          </p>
        </div>
